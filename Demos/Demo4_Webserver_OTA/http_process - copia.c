...
/////////////////PERMITE LA REPRODUCCIÓN/CREACIÓN DEL SERVIDOR PARA AUTENTICACION BÁSICA DE ACCESO A RECURSO//////////
...
///////////////////////////////////////////////////////////////////////////////////////////
		...
		//AQUI SE COMPARA SI SE SE HA PULSADO UN TIPO GET O POST EN EL SERVIDOR. Esto conducirá a send_system_page o a get_update post
		//strcmp devuelve 0 si ambas cadenas (httpToken.pToken1 y get) son iguales
	if(!strcmp(httpToken.pToken1, "GET"))  //Si la cadena httpToken.pToken1 es igual a la cadena get se entra en la funcion
	{//strlen devuelve el tamaño de cadena
    if(!strncmp(httpToken.pToken2, "/system.htm", strlen("/system.htm")))  {
			send_system_page(index);
		} //strncmp compara los primeros (c1,c2,x) x caracteres de ambas cadenas c1 y c2
		//creo que esto anterior nos llevaría a la página system.htm, en la que se dice que la config realizada es correcta de
		//parametros de red SSID y pass Config Done
...
//Lanza la pagina que sea, definida, del servidor, definidas en web_data.c ??
static void send_http_data(int index, char *data, int len)
{
